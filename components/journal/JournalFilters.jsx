...
const JournalFilters = ({ department = null }) => {
  const router = useRouter();

  const handleFilter = (slug) => {
    if (slug) {
      const toPath = slug !== "all" ? `/journals/department/${slug}` : '/journals'
      router.push(toPath);
    }
  }
        ...
        <div className="flex justify-between flex-col sm:flex-row gap-4">
            ...
            <SelectContent>
              <SelectItem value="all">All Departments</SelectItem>
              {
                DEPARTMENTS?.map(department => (
                  <SelectItem key={department?.id} value={department?.slug}>{department?.name}</SelectItem>
                ))
              }
            </SelectContent>
          </Select>
          ...
          <div className="flex-1 max-w-xl">
            <Suspense fallback={<SearchSkeleton />}>
              <SearchForm departmentSlug={department?.slug} />
            </Suspense>
          </div>
        </div>
      </div>
